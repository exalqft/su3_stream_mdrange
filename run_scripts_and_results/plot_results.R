...
for(i in 1:length(archs)) {
  ...
  dat <- read.table(sprintf("%s/results.dat", arch), header=TRUE) %>% dplyr::filter(!grepl("Plaquette", kernel) & kernel != "HalfStaple")
  ...
  p <- ggplot2::ggplot(dat , aes(x = n^4*4*9*16*10^(-6), y = bw, colour = kernel, shape = kernel)) +
       ...
       ggplot2::facet_wrap(sprintf("$n_{\\textrm{th}}=%03d$",nt) ~ sprintf("view = %s", SUN), ncol = ncol) +
  ...
  dat <- read.table(sprintf("%s/results.dat", arch), header=TRUE) %>% dplyr::filter(grepl("Plaquette", kernel) & kernel != "Plaquette-Trace")
  ncol <- length(unique(dat$SUN))
  maxdat <- dplyr::group_by(dat, nt, SUN) %>%
            dplyr::filter(n == max(n)) %>%
            dplyr::filter(bw == max(bw)) %>%
